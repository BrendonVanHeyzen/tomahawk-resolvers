...
var SubsonicPodcastsResolver = Tomahawk.extend(TomahawkResolver, {
    ...
    parseSongFromAttributes : function(song_attributes)
    {
        return {
            ...
            url:        this.buildBaseUrl("/rest/stream.view") + "&id=" + song_attributes["streamId"],
        ...
        Tomahawk.asyncRequest(search_url, function(xhr) {
            ...
            if (searchResults) {
                ...
                for (var i = 0; i < searchResults.length; i++) {
                    ...
                    var parsedResults = that.parseSongFromAttributes(searchResults[i]);
                    parsedResults['albumpos'] = i;
    ...
    {
        var results = [];
        artists_url += "&f=json&includeEpisodes=no"; //for large responses we surely want JSON
        ...
        Tomahawk.asyncRequest(artists_url, function(xhr) {
            ...
            Tomahawk.log("subsonic artists (podcasts) query:" + artists_url);
            Tomahawk.log("subsonic artists (podcasts) response:" + xhr.responseText);
            if (!!doc["subsonic-response"].podcasts) { // No search results yields empty string in 1.9.0 at least.
                var podcasts = doc["subsonic-response"].podcasts.channel;

                for (var i = 0; i < podcasts.length; i++)
                {
                  results.push(podcasts[i].title);
                  that.podcastIds[podcasts[i].title] =  podcasts[i].id;
                }
            }
            var return_artists = {
               qid: qid,
    ...
    executeAlbumsQuery : function(qid, search_url, artist)
    {
        ...
        Tomahawk.asyncRequest(search_url, function(xhr) {
            ...
            // var albums = doc["subsonic-response"].podcasts.channel[0].episode
    ...
    executeTracksQuery : function(qid, search_url, artist, album)
    {
        ...
        Tomahawk.asyncRequest(search_url, function(xhr) {
            ...
            var tracks = doc["subsonic-response"].podcasts.channel[0].episode
            ...
            if (tracks instanceof Array)
            {
                ...
                {
                    Tomahawk.log("tracks[i].title=" + tracks[i].title);
                    Tomahawk.log("artist=          " + artist);

                    var parsedResults = that.parseSongFromAttributes(tracks[i]);

                    parsedResults['artist'] = artist;
                    parsedResults['album'] = artist;
                    parsedResults['albumpos'] = i + 1;
    ...
    artists: function( qid )
    {
        ...
        var artists_url = this.buildBaseUrl("/rest/getPodcasts.view");
    ...
    albums: function( qid, artist )
    {
        ...
        var podcastId = this.podcastIds[artist];

        var search_url = this.buildBaseUrl("/rest/getPodcasts.view") + "&includeEpisodes=yes&id=" + podcastId;
    ...
    tracks: function( qid, artist, album )
    {
        ...
        var search_url = this.buildBaseUrl("/rest/getPodcasts.view") +
          "&includeEpisodes=yes&id=" + this.podcastIds[artist];
...
Tomahawk.resolver.instance = SubsonicPodcastsResolver;
